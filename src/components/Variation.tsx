...
export const Variation = ({ item, items2 }: {  item: Item, items2: Item[] }) => {
    ...
    const handleMatchFirstProjectVariation = async(featureFlagKey: string, variation: "onVariationValue" | "offVariationValue", value: boolean) => {
        ...
        const newMappedValues = items2State.map((item) => {
            if (item.key === featureFlagKey) {
                const index = item.variations.findIndex(variation => variation.value === value)
                const variationKey = variation === 'onVariationValue' ? 'onVariation' : 'offVariation'
    ...
    const getMappedVariationValue = (variation: number, variations: VariationElement[]): boolean => {
        return variations[variation].value
    }

    return <>
        <td className="text-center">
            {item.defaults.offVariation === offVariation ? <Match /> : <DoesNotMatch onClick={() => handleMatchFirstProjectVariation(item.key,
                "offVariationValue", getMappedVariationValue(item.defaults.offVariation, item.variations))}/>}
        </td>
        <td className="text-center">
            {item.defaults.onVariation === onVariation ? <Match /> :
                <DoesNotMatch onClick={() => handleMatchFirstProjectVariation(item.key,
                    "onVariationValue", getMappedVariationValue(item.defaults.onVariation, item.variations))} />}
        </td>
    </>
}
